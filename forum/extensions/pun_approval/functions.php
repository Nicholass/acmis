...
function extension_action()
{
	...
	$approve_multi = (isset($_POST['posts']) && is_array($_POST['posts'])) ? $_POST['posts'] : array();
	$errors = array();
	...
	if(isset($_GET['topics']) || isset($_POST['topics']))
	{
		if (isset($_GET['del']) || isset($_POST['del']))
			delete_unapproved_post($approve_multi);
		if (isset($_GET['app']) || isset($_POST['app']))
			approve_post($approve_multi);
	}
				...
				else
				{
					...
					?>
						...
						while ($cur_post = $forum_db->fetch_assoc($result))
						{
								...
								$forum_page['post_ident']['num'] = '<span class="post-num" style="margin-right:15px;">'.forum_number_format($forum_page['start_from'] + $forum_page['item_count']).'</span>';
								//ghost
								$forum_page['post_ident']['num'].= '<p class="item-select" style="margin-top:-1px;"><input type="checkbox" name="posts[]" value="'.$cur_post['id'].'" /></p>';
						...
						<div class="main-foot">
								...
								?>
								<h2 class="hn"><span><?php echo $forum_page['items_info'] ?></span><span style="float:right;"><input type="hidden" name="topics" value="1"><input type="submit" name="app" value="<?php echo $lang_app_post['Approve'];?>"><input type="submit" name="del" value="<?php echo $lang_app_post['Remove'];?>"></span></h2>
						</div>
						</form>
					<?php
				}
...
function delete_unapproved_post($approve_multi = array())
{
	global $forum_db,$lang_app_post,$forum_user,$ext_info, $lang_common, $forum_config;

	require $ext_info['path'].'/post_app_url.php';
	$pid = isset($_GET['del']) ? intval($_GET['del']) : 0;
	if ($pid < 1 && empty($approve_multi))
		message($lang_common['Bad request']);

	if (empty($approve_multi)) {
		$approve_multi = array($pid);
	}
...
foreach ($approve_multi as $pid) {

	//Ghost WTF? if (!$confirmation_required)
	{
		$query_app = array(
				'SELECT'	=> 'id',
				'FROM'		=> 'post_approval_topics',
...
redirect(forum_link($post_app_url['Permalink topic']), $lang_app_post['delete succesfull']);
}
...
function approve_post($approve_multi = array())
{
	...
	$query_app = array(
			...
			'JOINS'		=> array(
					array(
							'LEFT JOIN'	=> 'post_approval_topics AS t',
							'ON'			=> 't.first_post_id=p.id'
					)
			),
			'WHERE'		=> empty($approve_multi) ? 'p.id='.$pid : 'p.id IN (' . implode(',', $approve_multi) . ')' 
	);

	$result_all = $forum_db->query_build($query_app) or error(__FILE__, __LINE__);

	while ($res = $forum_db->fetch_assoc($result_all)) {

		$pid = $res['id'];
		$aptid = $res['tid'];
		if ($aptid) //if this post is a first post of some topic
		{
				$query_app_post = array(
	...
	redirect(forum_link($post_app_url['Section']), $lang_app_post['approve succesfull']);
}
