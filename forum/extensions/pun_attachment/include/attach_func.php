	...
	if ($forum_user['g_id'] == FORUM_ADMIN || $cur_posting['g_pun_attachment_allow_upload'] == 1)
	{
		...
		else
		{
			...
			if (empty($errors))
			{
				...
				if ($forum_user['g_id'] != FORUM_ADMIN && $uploaded_file['size'] > $cur_posting['g_pun_attachment_upload_max_size'] && !in_array($file_ext, array('png', 'jpg', 'jpeg')))
	...
	if (empty($errors))
	{
		...
		{		
            ...
            if ($forum_user['g_id'] != FORUM_ADMIN && $uploaded_file['size'] > $cur_posting['g_pun_attachment_upload_max_size']) {
                //Trying to resize the file

                $newfile = tempnam(FORUM_ROOT . $forum_config['attach_basefolder'].$forum_config['attach_subfolder'], 'tempimg');

                if (!move_uploaded_file($uploaded_file['tmp_name'], $newfile)) {
                ...
                if (empty($errors)) {
                    $resized = resize_attachment($newfile, $forum_config['attach_basefolder'] . $forum_config['attach_subfolder'] . '/' . $attach_name);

                    if (!$resized) {
                        $errors[] = sprintf($lang_attach['Filesize error'], $cur_posting['g_pun_attachment_upload_max_size']);
                    }

                    $uploaded_file['size'] = filesize($resized);
                }
            } else {
...
function show_attachments($attach_list, $cur_posting)
{
	...
	if (!empty($attach_list))
	{
		...
		{
			++$num;
			if (in_array($attach['file_ext'], array('png', 'jpg', 'jpeg', 'gif', 'tiff')) && $forum_config['attach_disp_small'] == '1')
			{
			...
			<div class="<?php echo $show_image ? 'ct-set' : 'sf-set'; ?> set<?php echo ++$forum_page['item_count'] ?>">
				...
				<?php if ($show_image):	?>
					<h3 class="hn ct-legend"><?php echo sprintf($lang_attach['Number existing'], $num).'&nbsp;'; ?></h3>
						<p class="show-image"><span><a href="<?php echo $download_link; ?>"><img width="<?php echo $attach['img_width'] > 660 ? '660' : $attach['img_width'].'px'; ?>" src="<?php echo $view_link; ?>" title="<?php echo forum_htmlencode($attach['filename']).', '.format_size($attach['size']).', '.$attach['img_width'].' x '.$attach['img_height']; ?>" alt="<?php echo $view_link; ?>"/></a></span></p>
	...
	if ($forum_user['g_id'] == FORUM_ADMIN || ($forum_user['g_pun_attachment_allow_upload'] && count($attach_list) < $forum_user['g_pun_attachment_files_per_post']))
	{
...
function resize_attachment($img_source, $newfile) {
    global $forum_config;
    ...
    if ($height <= $forum_config['attach_small_height'] && $width <= $forum_config['attach_small_width']) {
        $newwidth  = ($width <= $forum_config['attach_small_width'] ? $width : $forum_config['attach_small_width']);
        $newheight = ($height <= $forum_config['attach_small_height'] ? $height : $forum_config['attach_small_height']);
