...
function form_of_poll($question, $poll_answers, $options_count, $days_poll, $votes_poll, $read_unvote_users, $revote, $poll_open)
{
	...
	<fieldset class="frm-group group<?php echo ++$forum_page['group_count'] ?>">
		...
		<div class="mf-set set<?php echo ++$forum_page['item_count'] ?>">
			...
			<div class="mf-item">
				<br/>
				<span class="fld-input">
					<input id="fld<?php echo ++$forum_page['fld_count'] ?>" type="checkbox" name="poll_open" value="1" <?php echo $poll_open ? 'checked="checked"' : ''; ?>>
				</span><label for="fld<?php echo $forum_page['fld_count'] ?>">
						<?php echo $lang_pun_poll['Open description'] ?>
					</label>
			</div>
			</div>
		</div>
...
function add_poll($topic_id, $question, $poll_answers, $days_poll, $votes_poll, $read_unvote_users, $revote, $poll_open)
{
	global $forum_db;

	$query = array(
		'INSERT'	=>	'topic_id, question, read_unvote_users, revote, created, days_count, votes_count, open',
		'INTO'		=>	'questions',
		'VALUES'	=>	$topic_id.', \''.$forum_db->escape($question).'\', '.$read_unvote_users.', '.$revote.', '.time().', '.$days_poll.', '.$votes_poll.', '.$poll_open
	);
...
function update_poll($topic_id, $question, $poll_answers, $days_poll, $votes_poll, $read_unvote_users, $revote, $poll_open, $old_question, $old_poll_answers, $old_days_poll, $old_votes_poll, $old_read_unvote_users, $old_revote, $old_poll_open)
{
	global $forum_db;
	...
	if ($poll_open != $old_poll_open)
		$question_update_set[] = 'open = '.$poll_open;
...
function data_validation($question, &$poll_answers, &$poll_days, &$poll_votes, $read_unvote_users, $revote, $poll_open)
{
	...
	if (count($poll_answers) < 2 && !isset($_POST['update_poll']))
	...
	if ($poll_open !== FALSE)
	{
		if ($poll_open != 0 && $poll_open != 1)
			message($lang_common['Bad request']);
	}	
}

function is_vote_allowed() {
	global $cur_topic, $forum_user, $forum_db;
	
	//Is voting allowed in this forum?
	$query = array(
		'SELECT'	=> 'fp.vote',
		'FROM'		=> 'forum_perms AS fp',
		'WHERE'		=> 'fp.forum_id ='.$cur_topic['forum_id'].' AND fp.group_id ='.$forum_user['g_id']
	);
	
	$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);
	if (!$forum_db->num_rows($result)) {
		return $forum_user['g_vote'];
	} else {
		return $forum_db->result($result);
	}
}
