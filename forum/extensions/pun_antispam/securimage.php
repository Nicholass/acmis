...
class Securimage {
	/**
	* ACIS Captcha
	* (Dzen, Ghost, 2006-2009)
	*/
	var $use_my_captcha = true;
	var $word_index;
   ...
   * @var string
   */
  var $ttf_file = "./Zrnic_Cyr_Normal.ttf";
   ...
   */
  function show($background_image = "back1.jpg")
  {
    ...
    if ($this->use_gd_font == true) {
      ...
      if($this->use_transparent_text == true) {
        ...
        $font_color = imagecolorallocate($this->im, hexdec(substr($this->text_color, 1, 2)), hexdec(substr($this->text_color, 3, 2)), hexdec(substr($this->text_color, 5, 2)));
			...
			if ($this->use_my_captcha) {
				$start_x = ($this->image_width - strlen($this->code)*14)/2;
				if($start_x<7)$start_x=7;
				
				$img = $this->im;
				
				@imagettftext($img, $this->font_size, $angle, $start_x+rand(5,10), 25-rand(0,5),$font_color, $this->ttf_file, $this->code);

				// случайные параметры (можно поэкспериментировать с коэффициентами):
				// частоты
				$rand1 = mt_rand(700000, 1000000) / 19000000;
				$rand2 = mt_rand(700000, 1000000) / 50000000;
				$rand3 = mt_rand(700000, 1000000) / 10000000;
				$rand4 = mt_rand(700000, 1000000) / 9000000;
				// фазы
				...
				// амплитуды
				$rand9 = mt_rand(400, 800) / 100;
				$rand10 = mt_rand(400, 600) / 100;

				list($this->image_width, $this->image_height)=getimagesize("back1.jpg");
				$img2 = imagecreatetruecolor($this->image_width,$this->image_height);
				for($x = 0; $x < $this->image_width; $x++){
					for($y = 0; $y < $this->image_height; $y++){
						...
						if($sx < 0 || $sy < 0 || $sx >= $this->image_width - 1 || $sy >= $this->image_height - 1){
							$color = 255;
							$color_x = 255;
							$color_y = 255;
							$color_xy = 255;
				...
				$this->im = $img2;
				imagettftext($this->im, 12, 0, 5, 52, $font_color, $this->ttf_file, "Введите в текстовое поле  ".$this->word_index."-е  слово на картинке");
  ...
  {
		if ($this->use_my_captcha) {
			$phrases = @file($this->wordlist_file);
			
			if (count($phrases) == 0) return false;
				
			$phrase_index=rand(0,count($phrases)-1);
			if ($phrase_index != 10) {
				$phrase = rtrim($phrases[$phrase_index]);
			} else {
				$phrase = "х";
				$u_in_hui_count=rand(1,20);
				for($i=0;$i<$u_in_hui_count;$i++)
				{
					$phrase=$phrase."у";
				}
					$phrase=$phrase."й";
			}
			$this->word_index = rand(1, $this->word_count($phrase));
  ...
  function saveData()
  {
	  $_SESSION['pun_antispam_text'] = mb_strtolower($this->code, "UTF-8");
    $_SESSION['pun_antispam_text_index'] = $this->word_index;
  }
