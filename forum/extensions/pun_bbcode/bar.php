...
<?php
	...
	$tags_without_attr = array('b', 'i', 'u', 's', 'email', 'list', 'li' => '*', 'quote', 'code', 'url', 'tytruba' => 'youtube');
...
<?php
		...
		foreach (array_unique($smilies) as $smile_text => $smile_file)
		{
			...
			echo "\t\t".'<img src="'.$base_url.'/img/smilies/'.$smile_file.'" alt="'.$smile_text.'" onclick="insert_text(\' '.$smile_text.' \', \'\');" tabindex="'.($pun_bbcode_tabindex++).'" />'."\n";
		}
