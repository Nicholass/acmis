...
else if (isset($_GET['action']))
{
	...
	else
	{
		...
		if ($action == 'show_user_posts' || $action == 'show_user_topics' || $action == 'show_subscriptions')
		{
			...
			$value = (isset($_GET['forum'])) ? array_map('intval', $_GET['forum']) : array(-1);
...
<?php
		...
		else
		{
			...
			$forum_page['item_body']['subject']['desc'] = '<p class="item-subject-p">'.implode(' ', $forum_page['item_subject']).'</p>';
			...
			$forum_page['item_body']['info']['lastpost'] = '<li class="info-lastpost"><span class="label">'.$lang_forum['Last post'].'</span> <strong><a href="'.forum_link($forum_url['post'], $cur_set['last_post_id']).'">'.format_time($cur_set['last_post']).'</a></strong> '.sprintf($lang_forum['by poster'], forum_htmlencode($cur_set['last_poster'])).'</li>';
