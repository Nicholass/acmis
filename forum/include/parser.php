...
function preparse_bbcode($text, &$errors, $is_signature = false)
{
	...
	if ($is_signature)
	{
		...
		if (preg_match('#\[quote(=(&quot;|"|\'|)(.*)\\1)?\]|\[/quote\]|\[code\]|\[/code\]|\[youtube\]|\[/youtube\]|\[list(=([1a\*]))?\]|\[/list\]#i', $text))
	...
	if ($forum_config['p_sig_bbcode'] == '1' && $is_signature || $forum_config['p_message_bbcode'] == '1' && !$is_signature)
	{
		...
		// Remove empty tags
		while ($new_text = preg_replace('/\[(b|u|i|h|colou?r|quote|code|img|url|email|list|youtube)(?:\=[^\]]*)?\]\[\/\1\]/', '', $text))
		{
			if ($new_text != $text)
				$text = $new_text;
			else
...
function preparse_tags($text, &$errors, $is_signature = false)
{
	...
	$tags = array('quote', 'code', 'b', 'i', 'u', 'color', 'colour', 'url', 'email', 'img', 'list', '*', 'h', 'youtube');
	...
	$tags_quotes = array('url', 'email', 'img', 'youtube');
	...
	$tags_fix = array('quote', 'b', 'i', 'u', 'color', 'colour', 'url', 'email', 'h', 'youtube');
...
//
// Turns an Youtube from the [youtube] tag into an embedded video
//
function handle_youtube_tag($url)
{
    global $lang_common, $forum_user;

    $return = ($hook = get_hook('ps_handle_youtube_tag_start')) ? eval($hook) : null;
    if ($return != null)
        return $return;

    $pattern = '/(?:youtube(?:-nocookie)?\.com\/(?:[\w\-?&!#=,;]+\/[\w\-?&!#=\/,;]+\/|(?:v|e(?:mbed)?)\/|[\w\-?&!#=,;]*[?&]v=)|youtu\.be\/)([\w-]{11}([?&]*t=[\d]+)*)(?:[^\w-]|\Z)/i';

    $result = preg_match($pattern, $url, $matches);

    if ($result != 1) return null;

    $youtube_tag = '<iframe width="560" height="315" src="https://www.youtube.com/embed/'.$matches[1].'" allowfullscreen="allowfullscreen" frameBorder="0"></iframe>';

    $return = ($hook = get_hook('ps_handle_youtube_tag_end')) ? eval($hook) : null;
    if ($return != null)
        return $return;
...
function do_clickable($text)
{
	...
	$text = preg_replace('#(?<=[\s\]\)])(<)?(\[)?(\()?([\'"]?)(https?|ftp|news){1}://([\w\-]+\.([\w\-]+\.)*[\w]+(:[0-9]+)?(/[^\s\[]*[^\s.,?!\[;:-])?)\4(?(3)(\)))(?(2)(\]))(?(1)(>))(?![^\s]*\[/(?:url|img|youtube)\])#ie', 'stripslashes(\'$1$2$3$4\').handle_url_tag(\'$5://$6\', \'$5://$6\', true).stripslashes(\'$4$10$11$12\')', $text);
	$text = preg_replace('#(?<=[\s\]\)])(<)?(\[)?(\()?([\'"]?)(www|ftp)\.(([\w\-]+\.)*[\w]+(:[0-9]+)?(/[^\s\[]*[^\s.,?!\[;:-])?)\4(?(3)(\)))(?(2)(\]))(?(1)(>))(?![^\s]*\[/(?:url|img|youtube)\])#ie', 'stripslashes(\'$1$2$3$4\').handle_url_tag(\'$5.$6\', \'$5.$6\', true).stripslashes(\'$4$10$11$12\')', $text);
